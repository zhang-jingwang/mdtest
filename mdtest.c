    ...
    if (dirs) {
      if (create) {
        if (( rank == 0 )                                         &&
          ...
          printf("V-3: create dir: %d\n", itemNum+i);
          fflush(stdout);
        }

        //create dirs
        sprintf(curr_item, "%s/dir.%s%d", path, mk_name, itemNum+i);
        if (rank == 0 && verbose >= 3) {
       ...
       */
        ...
        if (( rank == 0 )                                       &&
          ...
          printf("V-3: remove dir: %d\n", itemNum+i);
          fflush(stdout);
        }

        //remove dirs
        sprintf(curr_item, "%s/dir.%s%d", path, rm_name, itemNum+i);
        if (rank == 0 && verbose >= 3) {
     ...
     */
      ...
      if (create) {
        if (( rank == 0 )                                             &&
          ...
          printf("V-3: create file: %d\n", itemNum+i);
          fflush(stdout);
        }

        //create files
        sprintf(curr_item, "%s/file.%s%d", path, mk_name, itemNum+i);
        if (rank == 0 && verbose >= 3) {
 ...
 */
       ...
       */
        ...
        if (( rank == 0 )                                       &&
          ...
          printf("V-3: remove file: %d\n", itemNum+i);
          fflush(stdout);
        }

        //remove files
        sprintf(curr_item, "%s/file.%s%d", path, rm_name, itemNum+i);
        if (rank == 0 && verbose >= 3) {
  ...
  if (rank == 0 && verbose >= 3) {
    ...
    if (!leaf_only || (leaf_only && currDepth == depth)) {
      if (collective) {
		...
		create_remove_items(++currDepth, dirs, create, collective, temp_path, (currDir*branch_factor)+1);
		currDepth--;
  ...
  if (( rank == 0 ) && ( verbose >= 1 )) {
    ...
    if (dirs) {
      if (rank == 0 && verbose >= 3 && (i%ITEM_COUNT == 0) && (i != 0)) {
        printf("V-3: stat dir: %d\n", i);
        fflush(stdout);
      }
      sprintf(item, "dir.%s%d", stat_name, item_num);
    } else {
      if (rank == 0 && verbose >= 3 && (i%ITEM_COUNT == 0) && (i != 0)) {
        printf("V-3: stat file: %d\n", i);
        fflush(stdout);
      }
...
void directory_test(int iteration, int ntasks, char *path) {
  ...
  if (( rank == 0 ) && ( verbose >= 1 )) {
    ...
    if (verbose >= 1 && rank == 0) {
        printf("V-1:   Directory creation: %10.3f sec, %10.3f ops/sec\n",
              t[1] - t[0], summary_table[iteration].entry[0]);
        printf("V-1:   Directory stat    : %10.3f sec, %10.3f ops/sec\n",
              t[2] - t[1], summary_table[iteration].entry[1]);
/* N/A
        printf("V-1:   Directory read    : %10.3f sec, %10.3f ops/sec\n",
              t[3] - t[2], summary_table[iteration].entry[2]);
*/
        printf("V-1:   Directory removal : %10.3f sec, %10.3f ops/sec\n",
...
void file_test(int iteration, int ntasks, char *path) {
  ...
  if (( rank == 0 ) && ( verbose >= 1 )) {
    ...
    if (verbose >= 1 && rank == 0) {
        printf("V-1:   File creation     : %10.3f sec, %10.3f ops/sec\n",
           t[1] - t[0], summary_table[iteration].entry[4]);
        printf("V-1:   File stat         : %10.3f sec, %10.3f ops/sec\n",
           t[2] - t[1], summary_table[iteration].entry[5]);
        printf("V-1:   File read         : %10.3f sec, %10.3f ops/sec\n",
           t[3] - t[2], summary_table[iteration].entry[6]);
        printf("V-1:   File removal      : %10.3f sec, %10.3f ops/sec\n",
...
int main(int argc, char **argv) {
    ...
    if (( rank == 0 ) && ( verbose >= 1 )) {
      ...
      fprintf( stdout, "items_per_dir           : %d\n", items_per_dir );
    ...
    for (i = first; i <= last && i <= size; i += stride) {
        ...
        for (j = 0; j < iterations; j++) {
            ...
            if (verbose >= 3 && rank == 0) {
              printf( "V-3: main (for j loop): making testdir, \"%s\"\n", testdir );
              fflush( stdout );
            }
#ifdef _HAS_PLFS
            if ( using_plfs_path ) {
            ...
            if (create_only) {
                ...
                if (verbose >= 1 && rank == 0) {
                    printf("V-1: main:   Tree creation     : %10.3f sec, %10.3f ops/sec\n",
            ...
            if (remove_only) {
                ...
                if (verbose >= 1 && rank == 0) {
                    printf("V-1: main   Tree removal      : %10.3f sec, %10.3f ops/sec\n",
        ...
        if (( rank == 0 ) && ( verbose >=2 )) {
          fprintf( stdout, "V-2: main (after for j loop): testdir is \"%s\"\n", testdir );
          fflush( stdout );
        }
